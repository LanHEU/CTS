...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if high_way:
            features = 32
            self.hwm = Generic_UNet(4, features, 1, 4, anchor_out=True, upscale_logits=True)
    ...
    def forward(self, x_data, x_t_c, timesteps, y=None):
        ...
        for ind, module in enumerate(self.output_blocks):
            if ind == 2:
                ...
                h = th.cat([h, hs.pop()], dim=1)
                h = module(h, emb)
                h = h + th.cat((de[1], de[1], de[1], de[1]),1).detach()  
            elif ind == 6:
                h = th.cat([h, hs.pop()], dim=1)
                h = module(h, emb)
                ...
                h = th.cat([h, hs.pop()], dim=1)
                h = module(h, emb)
                h = h + th.cat((de[3], de[3], de[3], de[3]),1).detach() 
            else:
