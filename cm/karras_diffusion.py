...
class KarrasDenoiser:
    ...
    #    x_st原始标签，x_t添加噪声
        ...
        model_output, denoised, cal = self.denoise(model, x_t, x_data, log, sigmas, **model_kwargs)
    ...
    def consistency_losses(
        ...
        x_data, x_st = th.split(x_start,4,dim=1)   #将传入的数据分成两份 数据和标签 x_data是数据
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
        ...
        if self.loss_norm == "l1":
            ...
            diffs_call = mean_flat((x_st - cal) ** 2)
            # loss_cal_t = mean_flat((x_st - cal_t) ** 2)
            # loss_cal = loss_cal_1 
            # diffs_call = (cal - cal_target+ 1e-44) ** 2
            loss_diff = mean_flat(diffs) * weights 
            loss_cal = diffs_call
            # loss_c = mean_flat((distiller - cal)**2)
            # print("loss_c",loss_c)
            # loss_cal = mean_flat(diffs_call) * weights
        ...
        terms["cal"] = loss_cal
        terms["loss"] = terms["diff"] + terms["cal"] *10 #+ terms["loss_cal"] * 0.1
...
def sample_onestep(
    ...
    s_in = x.new_ones([x.shape[0]])
    return distiller(x,x_data, sigmas[0] * s_in)
